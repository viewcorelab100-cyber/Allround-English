...
| 항목 | 결정 |
|------|------|
| 용도 | 사용 가이드/매뉴얼 (단방향, 댓글 없음) |
| 노출 위치 | 별도 페이지 `guide.html` (헤더 메뉴 진입) |
| 에디터 | WYSIWYG Rich Text Editor |
...
이 계획에 착수하기 전, 다음을 확정해 주세요.

1. **WYSIWYG 고수 vs 마크다운으로 변경**: 위 1장 권고를 본 후에도 WYSIWYG 유지하시겠습니까?
2. **카테고리 4종 시드**: '시작하기' / '강의 시청' / '결제·환불' / 'FAQ' 로 시작하면 충분한가요? 추가/수정 의견.
3. **slug 자동 생성 방식**: 한글 그대로(`/guide-detail.html?slug=강의-시청하는-법`) vs 영문 자동 번역 vs 숫자 ID. 한글 권장.
4. **임시저장 기능 필요 여부**: 있으면 Phase 2가 +30분. 단기적으로 PM이 작성할 양에 따라 결정.
5. **착수 시점**: 승인 즉시 vs 다른 진행 중 작업(`project_naver_login`, `project_new_design_publishing` 등) 마무리 후?

---

**이상. PM 검토 후 승인 부탁드립니다.**
